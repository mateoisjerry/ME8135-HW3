...
def update_weights():
    ...
    for i in range(N):
        particle_distances[i] = math.dist([particles[i][0],particles[i][1]], Zx[:2])
...
pygame.display.set_caption('Matthew Lisondra ME8135 Assignment 3 Q2a')
...
while not crashed:
    ...
    if (t%8)==0:
        ...
        pygame.draw.circle(gameDisplay, PINK, ([Zx[0]*1000,Zx[1]*400]),10) # scaled, Ht[2,2] row column is element = 2 which scales too large
